...
[horizontal]
*CLIENT INDEX*:: The index of the client to add the pet to must be supplied the user. +
...
The user chooses to add Garfield to client of index 1 of age 10 and is a male. Garfield is a cat and a tabby. +
* `addp c/1 pn/Scooby Doo pa/5 pg/M t/dog t/great_dane` +
The user chooses to add another pet called Scooby to client of index 1 Doo of age 5 and is a male. Scooby Doo is a dog and a great dane.
...
`add` `addp` `addapptto` `sortc` `sortp` `delete` `rmpetfrom` `rmapptfrom` `edit` `editp` `editappt` `clear`
====

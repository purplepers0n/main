...
[NOTE]
====
Redoable commands: those commands that modify VetterAppointments' content: +
...
[NOTE]
====
Undoable commands: those commands that modify VetterAppointments' content: +
`add` `addp` `addappttopet` `sortc` `sortp` `delete` `deletep` `rmapptfrompet` `edit` `editp` `editappt` `clear`
`schedule` `unschedule` `reschedule`
====
// end::undoredo[]
...
*Description:* Lists all the details of the client at the specified index, including his/her pets and appointments. +

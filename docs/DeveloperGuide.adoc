...
// tag::addpettoclient[]
=== Add Pet to Client Feature [Deprecated]

==== Reason for deprecation

The `AddPetToClientCommand` is deprecated and not implemented since v1.3 as the command implementation is not merged together with the `AddPetCommand`.

==== Implementation
The `AddPetToClientCommand` allows the user to add an existing pet to an existing client. A user can execute the command by providing the index of the pet and the index of the client in the following format `addpettoclient p/PET_INDEX c/CLIENT_INDEX`.
...
* **Alternative 1 (implemented choice):** Add a new command that extends `UndoableCommand`
...
* **Alternative 1 (implemented choice):** Use an association class `ClientOwnPet` to link `Pet` and `Client`
...
** Pros: Clickable tabs on the UI for easy toggling with both typing and mouse-click.
** Cons: `MainWindow` needs to handle the event to switch to the correct tab.
* **Alternative 2:** Have a single display without tabs, and update it with either Client, Pet or Vet Tech list when the `list`
command is executed
** Pros: Need not create a new event and handle it.
** Cons: Displayed list needs to be updated every time `list` command is executed, reducing efficiency.
// end::listperson[]
...
The following diagram shows the class diagram of the autocomplete implementation.

.AutoComplete Implementation Class Diagram
image::LogicAutoCompleteClassDiagram.png[width="800", caption="Figure 4-1. "]
...
** Pros: Does not involve other classes, easier for new developers to understand.
** Cons: Breaks the Single Responsibility Principle.

===== Aspect: Implementation of Algorithm

* **Alternative 1 (current choice):** Use a Trie data structure.
** Pros: Better software performance.
** Cons: Difficult to implement.
* **Alternative 2:** Iterate through all possible commands to find match prefix.
** Pros: Easier to implement.
** Cons: May result in performance loss if there are too many commands.
// end::autocomplete[]
...
[none]
* *Major Enhancement*: Support `listappt` and `listall` commands
+
[none]
** The `listappt` command will display all appointments in the main section of the application
...
** The `listall` command will take in a type parameter (either client, pet, vet technician or appointment)
and an index parameter of that type's list, and will then display all the associated information of
the particular item in a sidebar of the application. This is to easily see the relevant
details when wanting to check a particular item.
** The appropriate display on the UI will also be created for the above.
